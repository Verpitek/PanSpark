...
export class PanSparkVM {
  ...
  public getVariableMemory(): Map<string, Variable> {
    ...
    * This generates Arduino-compatible code from PanSpark instructions
    * Includes setup() and loop() functions with memory management
    */
   public transpileToArduino(code: string): string {
     const lines = code.split('\n');
     const arduinoCode: string[] = [];
     
     // Arduino sketch header
     arduinoCode.push('// Auto-generated Arduino sketch from PanSpark');
     arduinoCode.push('// DO NOT EDIT THIS FILE DIRECTLY');
     arduinoCode.push('');
     
     // Include statements
     arduinoCode.push('#include <Arduino.h>');
     arduinoCode.push('');
     ...
     arduinoCode.push('    if (strcmp(varNames[i], name) == 0) return i;');
     arduinoCode.push('  }');
     arduinoCode.push('  return -1;');
     arduinoCode.push('}');
     arduinoCode.push('');
     
     arduinoCode.push('// Helper function to create/update variable');
     arduinoCode.push('int setVariable(const char* name, float value) {');
     arduinoCode.push('  int idx = findVariable(name);');
     arduinoCode.push('  if (idx == -1) {');
     ...
     for (let i = 0; i < lines.length; i++) {
       ...
       switch (opcode) {
         ...
         case 'FOR': {
           ...
           const end = tokens[3];
           arduinoCode.push(`${procIndent}for (float ${varName} = ${start}; ${varName} <= ${end}; ${varName}++) {`);
           procIndent += '  ';
           break;
         }
         
         case 'ENDFOR': {
           procIndent = procIndent.slice(0, -2);
           arduinoCode.push(`${procIndent}}`);
           break;
         }
         ...
         case 'PROC': {
           // Define a function
           const procName = tokens[1];
           const args = tokens[2] || '';
           arduinoCode.push(`${procIndent}void ${procName}(${args}) {`);
           procIndent += '  ';
           inProc = true;
           break;
         }
         ...
         case 'CALL': {
           ...
           const procName = tokens[1];
           const args = tokens[2] || '';
           arduinoCode.push(`${procIndent}${procName}(${args});`);
           break;
         }
         
         case 'END': {
           arduinoCode.push(`${procIndent}return;`);
           break;
         }
         ...
         default: {
           // Pass through as comment for unknown opcodes
           arduinoCode.push(`${procIndent}// UNKNOWN: ${line}`);
           break;
         }
       }
     }
