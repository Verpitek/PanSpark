...
export const customOpCodes: Map<string, OpCodeHandler> = new Map();

export function registerOpCode(name: string, handler: OpCodeHandler) {
    customOpCodes.set(name.toUpperCase(), handler);
}
...
export function compile(code: string): Instruction[] {
  ...
  for (let counter = 0; counter < lines.length; counter++) {
    ...
    if (operation === undefined && customOpCodes.has(opName)) {
        operation = opName; // Use the string name for the custom opcode
    }
    
    // If it's still undefined, it's an unknown operation
    if (operation === undefined) {
        throw new Error(`Unknown operation "${tokens[0]}" at line ${counter + 1}`);
    }
...
export function* run(instructions: Instruction[]) {
  ...
  codeloop: while (counter < instructions.length) {
    ...
    if (typeof instruction.operation === 'string') {
      const handler = customOpCodes.get(instruction.operation);
      if (handler) {
        ...
        const context: InterpreterContext = {
          ...
          procVariableMemory,
          procLock,
          getVar: (name, line) => variableCheck(name, line),
          setVar: (name, value) => setVariableMemory(name, value),
        };
